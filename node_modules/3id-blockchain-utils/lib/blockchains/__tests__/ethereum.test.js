...
var sigUtils = _interopRequireWildcard(require("eth-sig-util"));
...
describe('Blockchain: Ethereum', () => {
  ...
  beforeAll(async () => {
    ...
    provider.manager.personal_sign = (data, address, callback) => {
      ...
      const result = sigUtils.personalSign(account.secretKey, {
        data
      });
  ...
  it('typeDetector: should detect eth address correctly', async () => {
    ...
    expect((await _ethereum.default.typeDetector(addresses[0], provider))).toEqual(_constants.ADDRESS_TYPES.ethereum);
  });
  it('isERC1271: should detect erc1271 address', async () => {
    expect((await _ethereum.default.isERC1271(addresses[0], provider))).toEqual(false);
    expect((await _ethereum.default.isERC1271(contractAddress, provider))).toEqual(true);
  });
  ...
  it('createLink: should create erc1271 proof correctly', async () => {
    ...
    expect((await _ethereum.default.createLink(testDid, contractAddress, _constants.ADDRESS_TYPES.erc1271, provider, {
      skipTimestamp: true
    }))).toMatchSnapshot();
  });
